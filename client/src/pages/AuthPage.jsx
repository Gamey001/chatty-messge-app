      ...
      <form className="auth-card" onSubmit={handleSubmit}>
        ...
        <div className="auth-switch">
          {mode === 'login' ? (
            <>
              New to WhisperBox?{' '}
              <a href="#" onClick={(e) => { e.preventDefault(); setMode('register'); setErrMsg(null); }}>
                Create an account
              </a>
            </>
          ) : (
            <>
              ...
              <a href="#" onClick={(e) => { e.preventDefault(); setMode('login'); setErrMsg(null); }}>
                Sign in
              </a>
            </>
          )}
        ...
        <div className="demo-section">
          <div className="demo-divider"><span>Or try the live demo</span></div>
          <div className="demo-hint">
            <FiUsers size={12} /> Open each button in a separate tab or browser, then chat between them.
          </div>
            ...
            {DEMO_USERS.map((u) => (
              ...
              >
                ...
                <span style={{ textAlign: 'left' }}>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>Sign in as {u.display_name.split(' ')[0]}</div>
                  <div style={{ fontSize: 11.5, color: 'var(--wb-text-muted)' }}>@{u.username}</div>
                </span>
              </button>
            ))}
          </div>
        ...
        <div style={{ marginTop: 22, fontSize: 12, color: 'var(--wb-text-muted)', display: 'flex', gap: 6, alignItems: 'center', justifyContent: 'center' }}>
          <FiLock size={12} />
